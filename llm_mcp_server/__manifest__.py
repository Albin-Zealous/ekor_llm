{
    ...
    "version": "17.0.1.3.5",
    "category": "Technical",
    "summary": "MCP server exposing eKor LLM tools to Claude Desktop and other MCP hosts",
    "description": """
        Model Context Protocol (MCP) Server for eKor LLM

        Enables Claude Desktop, Letta, and other MCP hosts to access and execute
        eKor tools directly through a standards-compliant MCP server.

        Core Features:
        • MCP 2025-06-18 protocol compliance with JSON-RPC 2.0
        • Bearer token authentication with eKor user integration
        • Stateful session management with concurrent request handling
        • Automatic tool discovery from llm.tool registry
        • Real-time tool execution with proper eKor context
